		...
		<div class="col-xl-12">
			...
			<div class="panel panel-primary">
				...
				<div class="panel-heading">
					<h4 class="panel-title">Update - Tendik</h4>
                ...
                <a href="{{ route('sa-tendik-index') }}" class="btn btn-warning">Kembali</a>
                    <br/><br/>
                <form method="POST" class="margin-bottom-0" action="{{ route('sa-tendik-update', ['id' => $tendik->id_tendik]) }}">
                        {{ csrf_field() }}
        
                            <label class="control-label">NIP <span class="text-danger">*</span></label>
                            <div class="row row-space-10 {{ $errors->has('nip') ? ' has-error' : '' }}">
                                <div class="col-md-12 m-b-15">
                                    <input type="text" id="nip" name="nip" class="form-control" placeholder="NIP" value="{{ $tendik->nip }}" required autofocus />
                                    @if ($errors->has('nip'))
                            ...
                            <div class="row row-space-10 {{ $errors->has('nama') ? ' has-error' : '' }}">
                                <div class="col-md-12 m-b-15">
                                    <input type="text" id="nama" name="nama" class="form-control" placeholder="Nama" value="{{ $tendik->nama }}" required autofocus />
                                    @if ($errors->has('nama'))
                                        <span class="help-block">
                            ...
                            <div class="row row-space-10 {{ $errors->has('alamat') ? ' has-error' : '' }}">
                                <div class="col-md-12 m-b-15">
                                    <input type="text" id="alamat" name="alamat" class="form-control" placeholder="Alamat" value="{{ $tendik->alamat }}" required />
                            ...
                            <div class="row row-space-10 {{ $errors->has('telepon') ? ' has-error' : '' }}">
                                <div class="col-md-12 m-b-15">
                                    <input type="text" id="telepon" name="telepon" class="form-control" placeholder="Telepon" value="{{ $tendik->telepon }}" required />
                                ...
                                <div class="col-md-12 m-b-15">
                                    <select class="form-control" name="jurusan" id="jurusan">
                                        ...
                                        @foreach($jurusan as $id => $nama)
                                            <option value="{{ $id }}" {{ $id == $tendik->id_jurusan ? 'selected' : '' }}>{{ $nama }}</option>
										@endforeach
