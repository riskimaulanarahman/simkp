...
@section('content')
	...
	<div class="row">
		...
		<div class="col-xl-12">
			...
			<div class="panel panel-primary">
				...
				<!-- end panel-heading -->
				<form action="{{ route('dosen.bimbingan') }}" method="GET" class="form-inline">
	...
	<div class="modal modal-message fade" id="modal-berkas">
		<div class="modal-dialog">
			<form method="post" action="{{ route('dosen.sendberkasdosen') }}"  enctype="multipart/form-data">
				{{ csrf_field() }}
	...
	<div class="modal modal-message fade" id="modal-bimbingan">
		<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">
						<form method="POST" class="margin-bottom-0" action="{{ route('dosen.bimbingan.updateacc') }}">
							{{ csrf_field() }}
							{{-- {{ $tahapan }} --}}
	...
	function dtlbimbingan(id_mhs,id_dosen,id_bimbingan_dosen,mhs,nim) {
		...
		$.getJSON(base_url+'/detail-bimbingan-mahasiswa/'+id_mhs+'/'+id_dosen,function(item){
			...
			$.each(item,function(x,y){
				...
				if(y.isReply !== 1) {
					var aksi = '<a href="/dosen-bimbingan-reply/'+y.id_bimbingan_dosen+'" class="btn btn-warning">Reply</a>';
				} else {
