	...
	<div class="row">
		...
		<div class="col-xl-12">
			...
			<div class="panel panel-primary">
				...
				<form action="{{ route('tendik.request.index') }}" method="GET" class="form-inline">
					...
					<table id="data-table-responsive" class="table table-striped table-bordered table-td-valign-middle">
						<thead>
							<tr>
								...
								<th width="text-nowrap">Berkas</th>
						...
						<tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $p->periodekp }}</td>
								<td>{{ $p->endperiode }}</td>
								<td><a href="{{ asset( '/data_berkas/' . $p->nama_file)  }}" target="_blank" data-toggle="tooltip" title="{{$p->nama_file}}" class="btn btn-success"><i class="fa fa-download"></i></a> 
                                <td>
									@if($p->request_status == 0)
                                    	<a href="{{ route('tendik.request.approval', ['id' => $p->id_approval ,'status' => 'approved'] ) }}" onclick="return confirm('Apakah anda yakin ?');" class="btn btn-warning">Approve</a>
                                    	<a href="{{ route('tendik.request.approval', ['id' => $p->id_approval ,'status' => 'rejected']) }}" onclick="return confirm('Apakah anda yakin Reject ?');" class="btn btn-danger">Reject</a>
